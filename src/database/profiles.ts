...
export const createProfile = async (
  profileData: Prisma.ProfileUncheckedCreateInput
) => {
  try {
...
export const getProfileByUserId = (userId: string) =>
  prisma.profile.findUnique({ where: { userId }, include: includeAll });
...
export const updateProfileById = (
  ...
  prisma.profile.update({ where: { id }, data: profile, include: includeAll });
...
export const addStyleToProfileByUserId = (id: string, styleId: number) =>
  prisma.profile.update({
    where: { userId: id },
    data: {
      styles: {
...
export const removeStyleToProfileByUserId = async (
  userId: string,
  styleId: number
) => {
  try {
...
export const getGenderById = (id: number) =>
  prisma.gender.findUnique({ where: { id } });
...
export const createGender = async (
  genderData: Prisma.GenderUncheckedCreateInput
) => {
  try {
...
export const createSilhouette = async (
  silhouetteData: Prisma.SilhouetteUncheckedCreateInput
) => {
  try {
...
export const createStyle = async (
  styleData: Prisma.StyleUncheckedCreateInput
) => {
  try {
